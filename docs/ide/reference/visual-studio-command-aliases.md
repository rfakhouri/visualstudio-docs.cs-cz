...
ms.locfileid: "31952942"
---
# <a name="visual-studio-command-aliases"></a>Aliasy příkazů sady Visual Studio

Aliasy příkazů umožňují zadejte menší počet znaků, pokud chcete provést příkaz. Zadejte aliasy do **najít/příkaz** pole nebo **příkaz** okno. Například místo zadávání `>File.OpenFile` zobrazíte **otevření souboru** dialogové okno, můžete použít předdefinované alias `>of`.
...
|[Seznam paměť – příkaz](../../ide/reference/list-memory-command.md) jako ANSI pomocí čtyř bajtů formátu|řadič domény|Debug.listmemory – /Format:FourBytes /Ansi|
|[Seznam paměť – příkaz](../../ide/reference/list-memory-command.md) formátu čtyř bajtů|dd|Debug.listmemory – /Format:FourBytes|
