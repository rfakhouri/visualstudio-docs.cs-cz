...
- NuGet je systém správy balíčků open source, který slouží ke zjednodušení procesu začlenění knihoven do projektu řešení. Další informace najdete v tématu [NuGet dokumentaci](http://docs.microsoft.com/nuget).
...
|MSBuild automaticky spotřebuje sestavení a soubory metadat (.winmd) systému Windows.|A|V sadě SDK automaticky předáno kompilátoru.|A||
|MSBuild automaticky spotřebuje souborů .h nebo lib.|A|*SDKName*soubor PROPS informuje Visual Studio, jak nastavit adresář Visual C++ a tak dále, pro automatické používání souborů .h nebo lib.|N||
|MSBuild automaticky využívá soubory .js nebo .css.|A|V **Průzkumníku řešení**, můžete rozbalit uzel odkazu JavaScript SDK a zobrazit jednotlivé .js nebo .css soubory a pak generovat `<source include/>` značky přetažením těchto souborů do jejich zdrojové soubory. SDK podporuje F5 a automatické balíček instalačního programu.|A||
...
|Tento mechanismus můžete zadat "nechcete"kopie.|A|V závislosti na tom, jestli jsou soubory vyřadit ve složce \redist nebo \designtime můžete řídit soubory, které chcete zkopírovat do balíčku spotřebitelskou aplikací.|N|Je deklarovat soubory, které chcete kopírovat v manifest balíčku.|
...
|Tento mechanismus podporuje zadání registrační informace pro nativní WinMDs.|A|Korelace mezi .winmd soubor a soubor .dll můžete zadat v SDKManifest.xml.|N||
...
|Tento mechanismus nasadí soubory – referenční dokumentace (například nasadit test framework, při kterém se má spouštět testy [!INCLUDE[win8_appname_long](../debugger/includes/win8_appname_long_md.md)] aplikace).|A|Pokud je ve složce \redist vyřadit soubory, soubory se automaticky nasadí.|A||
...
|Tento mechanismus obsahuje soubor .exe samostatné pro vytváření a použití balíčků.|A|Sada SDK obsahuje MSBuild.exe.|A||
...
|Můžete vytvořit balíček Symbol pro ladění podpory.|A|Pokud je vyřadit soubory PDB v sadě SDK, soubory získat převzata automaticky.|A||
...
|Tento mechanismus je k dispozici pro všechny edice sady Visual Studio.|A|Sada SDK podporuje všechny verze sady Visual Studio, z Visual Studio Express prostřednictvím [!INCLUDE[vsUltLong](../code-quality/includes/vsultlong_md.md)].|A|NuGet podporuje všechny edice sady Visual Studio Express až prostřednictvím [!INCLUDE[vsUltLong](../code-quality/includes/vsultlong_md.md)].|
