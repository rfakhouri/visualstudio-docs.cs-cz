---
title: "Krok 2: Vytvořte náhodný problém s přidáním | Microsoft Docs"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
4.  Přidejte do formuláře dvě proměnné celé číslo a název je **addend1** a **addend2**.  
    ...
    >  Proměnná s celým číslem se označuje jako int v C# nebo celé číslo v jazyce Visual Basic. Tento druh proměnné ukládá kladné a záporné číslo od -2147483648 až 2147483647 a můžete uložit jenom celá čísla, nikoli desetinná místa.  
     ...
     Můžete použít podobné syntaxe pro přidání proměnná typu integer, jako jste to udělali pro přidání `Random` objekt, jak ukazuje následující kód.  
  
     [!code-csharp[VbExpressTutorial3Step2#2](../ide/codesnippet/CSharp/step-2-create-a-random-addition-problem_2.cs)]
     [!code-vb[VbExpressTutorial3Step2#2](../ide/codesnippet/VisualBasic/step-2-create-a-random-addition-problem_2.vb)]  
  
5.  Přidejte metodu, která je s názvem `StartTheQuiz()` a používající `Random` objektu `Next()` metoda zobrazíte náhodná čísla v štítky. `StartTheQuiz()`bude nakonec vyplní všechny problémy a pak spustit časovač, proto přidejte komentář. Funkce by měla vypadat podobně jako tento.  
  
     [!code-csharp[VbExpressTutorial3Step2#3](../ide/codesnippet/CSharp/step-2-create-a-random-addition-problem_3.cs)]
     [!code-vb[VbExpressTutorial3Step2#3](../ide/codesnippet/VisualBasic/step-2-create-a-random-addition-problem_3.vb)]  
  
     Všimněte si, že když zadáte tečku (.) po **randomizer** v kód, otevře se okno technologie IntelliSense a jsou zobrazeny všechny `Random` objektu metody, které můžete volat. Například Intellisense zobrazí seznam `Next()` metoda následujícím způsobem.  
  
     ![Next – metoda](../ide/media/express_randomwhite.png "Express_RandomWhite")  
Next – metoda  
...
6.  V okně návrhu buď poklikejte **spustit** tlačítko, nebo zvolte jej a potom zvolte klávesu Enter.  
...
8.  Uložit kódu, spusťte ji a pak zvolte **spustit** tlačítko.  
     ...
     ![Náhodný problém s přidáním](../ide/media/express_additionproblem.png "Express_AdditionProblem")  
Náhodný problém s přidáním  
...
-   Chcete-li vrátit k předchozímu kroku kurzu, přečtěte si téma [krok 1: Vytvořte projekt a přidejte popisky vaše formuláře](../ide/step-1-create-a-project-and-add-labels-to-your-form.md).