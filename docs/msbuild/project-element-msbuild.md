...
|`SDK`|Nepovinný atribut. (K dispozici pouze pro .NET Core projektů Visual Studio 2017 nebo novější).<br /><br /> Verze sady SDK, který se má použít k vytvoření implicitní Import příkazy, které jsou přidány do souboru .proj. Například `<Project Sdk="Microsoft.NET.Sdk/1.0.0-RC" />`.|  
