...
-   Pokud vytvoříte typ MIME s příponou "*" a "application/octet-stream" typ MIME bude možné soubory typu odblokuje soubor ke stažení. (Ale typy souborů, jako blokované *.aspx* a *.asmx* nelze stáhnout.)  
...
## <a name="content-type-mappings"></a>Mapování typu obsahu  
 ...
 Pokud nasadíte pomocí serveru služby IIS, spusťte *inetmgr.* soubor EXE a přidejte nový typ "application/x-ms-application" obsahu pro *.application* rozšíření.  
