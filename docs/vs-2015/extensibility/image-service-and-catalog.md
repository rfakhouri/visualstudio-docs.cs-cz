...
2. Změnit všechny atributy zdroje Moniker atributy  
...
1. Na kartě okna nástroje když karty získat malé dostatek (také použít přepínač okna Ctrl + Tab).  
...
2. Příkaz pro otevření okna nástrojů.  
    ...
    V souboru .vsct balíčku upravte panel nástrojů příkazového tlačítka:  
...
3. Na kartě okna nástroje když karty získat malé dostatek (také použít přepínač okna Ctrl + Tab).  
   ...
   2. Viz krok #1 "Jak na to použít obraz Monikery v novém okně nástroje?" výše uvedené části.  

4. Příkaz pro otevření okna nástrojů.  

   - Viz krok #2 "Jak na to použít obraz Monikery v novém okně nástroje?" výše uvedené části.  

## <a name="how-do-i-use-image-monikers-in-a-vsct-file"></a>Použití zástupných názvů image v souboru .vsct  
 Aktualizace souboru .vsct, jak je uvedeno ve níže uvedené komentářem řádky:  
 ...
 **Co když Moje souboru .vsct musí také čtení ve starších verzích sady Visual Studio?**  
 ...
 Starší verze sady Visual Studio nedokáže rozpoznat **IconIsMoniker** příkazu příznak. Ve verzích sady Visual Studio, které ho podporují, ale pokračovat v používání starého typu Image ke starším verzím sady Visual Studio můžete použít Image z image služby. K tomuto účelu by souboru .vsct nechte beze změny (a tedy kompatibilní se staršími verzemi sady Visual Studio) a vytvořte soubor CSV (hodnoty oddělené čárkami), která se mapuje z dvojice GUID a ID definované v souboru .vsct \<rastrové obrázky > element do bitové kopie moniker páry/ID identifikátoru GUID.  
 ...
 **IconMappingFilename** je relativní cesta implicitně kořenovým adresářem v $PackageFolder$ (jako v příkladu výše) nebo absolutní cesta explicitně kořenovým adresářem v adresáře určené proměnnou prostředí, jako je například @"%UserProfile%\ dir1\dir2\MyMappingFile.csv".  
...
3. Změňte implementaci původní VSHPROPIDs u ikon, vytvořit "starší" verze ikony, pokud Rozšiřovací body o ně požádat. **IVsImageService2** poskytuje funkce potřebné k získání těchto ikon  
   ...
   **Pomocí rozšíření systému projektu sady SDK**  
   ...
   Postupujte podle pokynů na adrese [zadejte vlastní ikony pro typ typem projektu/položky](https://github.com/Microsoft/VSProjectSystem/blob/master/doc/scenario/provide_custom_icons_for_the_project_or_item_type.md) přizpůsobení Image CPS. Další informace o prohlášení CPS najdete [dokumentace rozšíření systému projektu sady Visual Studio](https://github.com/Microsoft/VSProjectSystem)  
...
2. Pokud používáte pouze **KnownMonikers**, proveďte následující kroky:  
   ...
   - Přidejte < Import Manifest="$(ManifestFolder)\\< nainstalovat relativní cestu k domovskému adresáři na\>\Microsoft.VisualStudio.ImageCatalog.imagemanifest" /\> k hornímu okraji \<symboly > oddílu.  
...
## <a name="testing-your-images"></a>Testování obrázků  
 Nástroj Prohlížeč knihovny obrázků můžete použít k testování vaše image manifesty, abyste měli jistotu, že všechno je správně nastavená oprávnění. Nástroj v [Visual Studio 2015 SDK](https://msdn.microsoft.com/library/bb166441.aspx). Pro tento nástroj a další dokumentaci najdete [tady](https://aka.ms/VSImageThemeTools).  
 ...
 **ManifestToCode**  
 ...
 Manifest do kódu – nástroj přebírá soubor manifestu image a vytvoří soubor obálky pro odkazování na manifestu hodnoty v kódu (C++, C# nebo VB) nebo soubory .vsct.  

 **ImageLibraryViewer**  

 Nástroj Prohlížeč knihovny obrázků můžete načíst obrázek manifestů a umožňuje uživatelům s nimi manipulovat stejným způsobem, který Visual Studio by zajistit, aby že manifest správně vytvořena. Uživatel může změnit na pozadí, velikosti, nastavení DPI, vysoký kontrast a další nastavení. Také zobrazuje načítání informace pomáhají najít chyby v manifestech a zobrazí informace o zdroji pro každý obrázek v manifestu.  
...
- Aktualizuji CPS systému projektu. Co se stalo s **ImageName** a **StockIconService**?  
    ...
    - o tyto byly odebrány při aktualizaci CPS použití zástupných názvů. Už nemusíte volat **StockIconService**, stačí pouze předat požadovaný **KnownMoniker** metoda nebo vlastnost pomocí **ToProjectSystemType()** metody rozšíření v prohlášení CPS nástroje. Můžete tu také mapování z **ImageName** k **KnownMonikers** níže:  
