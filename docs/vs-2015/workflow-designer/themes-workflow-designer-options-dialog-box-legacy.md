---
title: Dialogové okno Možnosti motivy, Návrhář pracovního postupu (starší verze) | Dokumentace Microsoftu
ms.custom: ''
ms.date: 2018-06-30
ms.prod: .net-framework-4.6
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: reference
f1_keywords:
- VS.ToolsOptionsPages.WorkflowDesigner.Themes
helpviewer_keywords:
- Options dialog box, workflow themes
- themes, options
ms.assetid: 71150631-4674-4568-bce8-570bef3780ad
caps.latest.revision: 6
author: gewarren
ms.author: gewarren
manager: erikre
ms.openlocfilehash: 12d7155d41bd22832dc40526258911f844f59f40
ms.sourcegitcommit: 55f7ce2d5d2e458e35c45787f1935b237ee5c9f8
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 08/22/2018
ms.locfileid: "42696206"
---
# <a name="themes-workflow-designer-options-dialog-box-legacy"></a>Dialogové okno Motivy, Návrhář postupu provádění, Možnosti (starší verze)
Toto téma popisuje, jak používat **motivy, Návrhář postupu provádění, možnosti** dialogové okno v starší [!INCLUDE[wfd1](../includes/wfd1-md.md)]. Použijte starší [!INCLUDE[wfd2](../includes/wfd2-md.md)] potřeba cílit na platformu [!INCLUDE[netfx35_long](../includes/netfx35-long-md.md)] nebo [!INCLUDE[vstecwinfx](../includes/vstecwinfx-md.md)].  
  
 Motiv určuje barvu pozadí a popředí, styly, ikony a další vizuální prvky pracovního postupu. Motivy pro opakované použití můžete šetřit tak další pracovní postupy. Můžete vybrat výchozí a pomocí uloženého vlastní motivy **motivy, Návrhář postupu provádění, možnosti** dialogové okno. Chcete-li otevřít toto dialogové okno, klikněte na tlačítko **vyberte vlastní motiv** na **pracovního postupu** nabídky. Nebo klikněte pravým tlačítkem na povrchu návrhu pracovního postupu a klikněte na tlačítko **vyberte vlastní motiv**.  
  
 Následující tabulka popisuje prvky uživatelského rozhraní (UI) **motivy, Návrhář postupu provádění, možnosti** dialogové okno.  
  
|Prvek uživatelského rozhraní (UI)|Popis|  
|----------------|-----------------|  
|**Nový**|Kliknutím otevřete [dialogové okno Konfigurace motivu (starší verze)](../workflow-designer/theme-configuration-dialog-box-legacy.md) vytvořit nový motiv.|  
|**Upravte...**|Kliknutím otevřete [dialogové okno Konfigurace motivu (starší verze)](../workflow-designer/theme-configuration-dialog-box-legacy.md) k úpravě vybraného motivu.|  
|**Procházet**|Klikněte na tlačítko Procházet pro soubor motivu pracovního postupu, který používá příponu názvu souboru .wtm.|  
|**Vyberte motiv**|Záhlaví sloupce název motivu motivy seznamu. Vyberte motiv ze seznamu, klikněte na tlačítko **změnit** upravit, nebo klikněte na **OK** používat vybraného motivu pracovního postupu.|  
|**Popis**|Motiv záhlaví sloupce Popis seznamu motivů.|  
|**Poloha**|Záhlaví sloupce umístění motivu motivy seznamu.|  
  
## <a name="see-also"></a>Viz také  
 [Dialogové okno Konfigurace motivu (starší verze)](../workflow-designer/theme-configuration-dialog-box-legacy.md)   
 [Nápověda k uživatelskému rozhraní návrháře pro programovací model Windows Workflow Foundation](../workflow-designer/legacy-designer-for-windows-workflow-foundation-ui-help.md)