...
1. Vytvoření [!INCLUDE[dsl](../includes/dsl-md.md)] řešení, které obsahuje alespoň dvě třídy, které v tomto příkladu se nazývají **knihy** a **Autor**. Měla by existovat relace mezi buď typu **knihy** a **Autor**.  
...
```  
 ...
 Můžete zadat vlastní editor nebo v editoru [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)], jako například <xref:System.Windows.Forms.Design.FileNameEditor> nebo <xref:System.Drawing.Design.ImageEditor>. Například použijte následující postup vlastnosti, ve kterém může uživatel zadat název souboru.  
