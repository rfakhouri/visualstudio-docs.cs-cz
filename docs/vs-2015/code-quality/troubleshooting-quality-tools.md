---
title: Řešení potíží s nástroji kvality | Dokumentace Microsoftu
ms.custom: ''
ms.date: 2018-06-30
ms.prod: visual-studio-dev14
ms.reviewer: ''
ms.suite: ''
ms.technology:
- vs-ide-general
ms.tgt_pltfrm: ''
ms.topic: article
helpviewer_keywords:
- Visual Studio ALM, developing the application
ms.assetid: 535b9e67-ce9e-4a3e-8d28-9365f257036e
caps.latest.revision: 19
author: erickson-doug
ms.author: gewarren
manager: douge
ms.openlocfilehash: aa55be74680d9961354bc8e2c4ccd92daab33ec1
ms.sourcegitcommit: 55f7ce2d5d2e458e35c45787f1935b237ee5c9f8
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 08/22/2018
ms.locfileid: "42669958"
---
# <a name="troubleshooting-quality-tools"></a>Řešení potíží s nástroji kvality
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

Nejnovější verzi tohoto tématu můžete najít v [řešení potíží s nástroji kvality](https://docs.microsoft.com/visualstudio/code-quality/troubleshooting-quality-tools).  
  
Pokud máte problémy při spuštění nástroje sady Visual Studio kvality, témata v této části můžete vám pomohou diagnostikovat a opravit problém.  
  
## <a name="in-this-section"></a>V tomto oddílu  
 [Řešení potíží s Analýzou kódu](../code-quality/troubleshooting-code-analysis-issues.md)  
  
 [Řešení potíží s metrikami kódu](../code-quality/troubleshooting-code-metrics-issues.md)



