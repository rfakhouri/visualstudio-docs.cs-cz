...
Tento článek popisuje, jak můžete použít Visual Studio pro zpracování 3D prostředky a zahrnout je ve vašich sestaveních.

Po použití nástroje v sadě Visual Studio k vytvoření 3D prostředky, dalším krokem je jejich použití ve vaší aplikaci. Ale před použitím je, vaše prostředky musí být uvedeny ve formátu, který můžete porozumět DirectX. Chcete-li transformace vaše prostředky, Visual Studio poskytuje sestavení vlastní nastavení pro jednotlivé typy asset, který může vytvářet. Zahrnout prostředky buildu, všechny, které musíte udělat, je nakonfigurujete svůj projekt použijte vlastní nastavení sestavení, přidat prostředky do projektu a nakonfigurovat prostředky používat vlastní nastavení správná sestavení. Potom můžete načíst prostředky do vaší aplikace a použít je pomocí vytvoření a naplnění DirectX prostředky stejně jako v jakékoli jiné DirectX aplikaci.

## <a name="configure-your-project"></a>Konfigurace projektu

Před nasazením vaše 3D prostředky v rámci vaší sestavení sady Visual Studio se chcete dozvědět o typech prostředků, které chcete nasadit. Visual Studio už ví o mnoho běžné typy souborů, ale protože pouze určité typy aplikací použít 3D prostředky, Visual Studio nepodporuje předpokládá, že projektu budete stavět tyto typy souborů. Se dá zjistit sady Visual Studio, vaše aplikace používá tyto typy prostředků pomocí *přizpůsobení sestavení*– soubory, které informují Visual Studio pro zpracování různých typů souborů užitečným způsobem – které jsou k dispozici pro každý typ prostředku. Protože tato přizpůsobení budou použita na jednotlivých projektů, je všechny, které musíte udělat, do projektu přidejte příslušné úpravy.
...
## <a name="include-assets-in-your-build"></a>Zahrnout prostředky v buildu
...
3.  V části **vlastnosti konfigurace**, zvolte **Obecné**a poté v mřížku vlastností v rámci **Obecné**, nastavte **typ položky** vlastnost Typ položky odpovídající obsahu kanálu. Například vyberte soubor obrazu nebo texture **bitové kopie obsahu kanálu**.
    ...
    > [!IMPORTANT]
    > Ve výchozím nastavení, Visual Studio se předpokládá, že různé druhy soubory obrázků by měl být zařazené do kategorie pomocí **Image** typ, který je integrován v sadě Visual Studio položky. Proto je nutné změnit **typ položky** vlastnost každé bitové kopie, kterou chcete zpracovat kanálu obsahu bitové kopie. Jiné typy obsahu kanálu zdrojové soubory pro 3D modely a výchozí grafiky visual shaderu na správnou **typ položky**.
...
## <a name="configure-asset-content-pipeline-properties"></a>Konfigurovat vlastnosti asset obsahu kanálu
...
## <a name="load-and-use-3d-assets-at-run-time"></a>Načtení a použití 3D majetku za běhu

### <a name="use-textures-and-images"></a>Použití textury a obrázků
...
### <a name="use-shaders"></a>Použití shadery
