---
title: Zdrojový kód L2DBForm.XAML | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- vs-ide-designers
...
## <a name="window-resource-section"></a>Části okna prostředků  
 ...
 `<ObjectDataProvider>` Deklaruje značku, která zahrnuje řádky 11 až 25, <xref:System.Windows.Data.ObjectDataProvider>s názvem `LoadedBooks`, která používá <xref:System.Xml.Linq.XElement> jako zdroj. To <xref:System.Xml.Linq.XElement> se inicializuje pomocí analýza embedded dokumentů XML ( `CDATA` element). Všimněte si, že mezer se zachová, i když deklarace embedded dokumentu XML a i když je analyzována. K tomu bylo potřeba, protože <xref:System.Windows.Controls.TextBlock> řízení, které se používá k zobrazení nezpracované XML, nemá žádné speciální XML možnosti formátování.  
...
```  
 ...
 Díky tomu je možné (na řádku 46) pro <xref:System.Windows.Controls.TextBlock> s názvem `tbRawXml` chcete zobrazit nezpracované XML podle vazby pro tohoto zprostředkovatele dat `Xml` vlastnost:  
...
```  
 ...
 Pak použije obousměrný datovou vazbu, tak, aby aktuální hodnoty elementů adresáře se zobrazují na a aktualizovat ze dvou textových polí v tomto panelu. Datová vazba na dynamické vlastnosti je podobná používány `BookTemplate` data šablony:  
  
```  
Text="{Binding Path=Attribute[id].Value}"...Text="{Binding Path=Value}"  
```  
  
 Poslední část uživatelského rozhraní, **přidat nové knihy**, nepoužívá datové vazby v jeho kód XAML; místo toho takový kód naleznete v jeho zpracování kód v souboru L2DBForm.xaml.cs událostí.  
