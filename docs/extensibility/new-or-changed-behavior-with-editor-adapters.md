...
#### <a name="use-win32-and-wpf-versions-of-ivstextview"></a>Verze IVsTextView použití Win32 a WPF
 ...
 Zobrazení textu nastavit na režim WPF, musíte volat <xref:Microsoft.VisualStudio.TextManager.Interop.IVsTextView.Initialize%2A> a předejte mu [TextViewInitFlags3.VIF_NO_HWND_SUPPORT](<xref:Microsoft.VisualStudio.TextManager.Interop.TextViewInitFlags3.VIF_NO_HWND_SUPPORT>) jako jeden z inicializace příznaky `InitView` parametru. Můžete získat <xref:System.Windows.FrameworkElement> voláním <xref:Microsoft.VisualStudio.Shell.Interop.IVsUIElementPane.CreateUIElementPane%2A>.
