...
1.  Klikněte pravým tlačítkem na uzel projektu v Průzkumníku řešení a zvolte **přidat**, **novou položku...** . V levém podokně pod uzlem C# vyberte **Data** a v prostředním podokně vyberte **ADO.NET Entity Data Model**.
  ...
  2.  Volání modelu `Northwind_model` a na tlačítko OK. **Entity Data Model Wizard** otevře. Zvolte **EF Designer z databáze** a pak klikněte na **Další**.
...
5.  Průvodce vytvoří třídy C#, které představují model Entity Framework. Třídy jsou prostý staré třídy jazyka C# a jsou co jsme databind do uživatelského rozhraní grafického subsystému WPF. Soubor EDMX popisuje vztahy a další metadata, která přidruží třídy objektů v databázi. Soubory .tt T4 šablony, které generují kód, který funguje na modelu a uložte změny do databáze. Zobrazí se všechny tyto soubory v Průzkumníku řešení v uzlu Northwind_model:
...
6.  Soubory .tt jsou obecné účely a je potřeba upravit jeden z nich pro práci s datové vazby WPF, který vyžaduje ObservableCollections. V Průzkumníku řešení rozbalte uzel Northwind_model vyhledejte Northwind_model.tt. (Zajistěte, aby se *není* v *. Kontext .tt soubor, který je přímo pod soubor EDMX.)
...
Výchozí uspořádání vytvářených Visual Studio není ideální pro naši aplikaci, takže jsme budete provádět některé změny ručně v XAML. Také je potřeba některé "forms" (které jsou ve skutečnosti mřížky) zajistit, aby uživatel k přidání nového zákazníka nebo pořadí. Aby bylo možné přidat nové zákazníka a pořadí, potřebujeme samostatnou sadu textová pole, které nejsou vázané na data na `CollectionViewSource`. Jsme budete řídit mřížky, které uživateli se zobrazí v každém okamžiku nastavením vlastnosti viditelné v metodách obslužné rutiny. Nakonec přidáme tlačítko Odstranit pro každý řádek v mřížce objednávky umožňující uživateli odstranit jednotlivé pořadí.
...
V aplikacích Windows Forms načíst objekt BindingNavigator s tlačítka pro procházení řádků v databázi a provádění základních operací CRUD. WPF neposkytuje BindingNavigator, ale je dostatečně snadné ji vytvořit. Provedeme to pomocí tlačítka uvnitř vodorovné StackPanel a jsme budete tlačítka přidružit příkazy, které jsou vázány na metody v kódu.
