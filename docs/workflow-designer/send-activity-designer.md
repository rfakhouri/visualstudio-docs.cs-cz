...
| <xref:System.ServiceModel.Activities.Send.OperationName%2A> | Pravda | Název operace služby volané to <xref:System.ServiceModel.Activities.Send> aktivity. Tato vlastnost se používá k sestavení kompletních výchozí hodnota **akce** vlastnost Pokud **akce** není explicitně nastavena vlastnost. |
| <xref:System.ServiceModel.Activities.Send.ServiceContractName%2A> | Pravda | Název kontraktu služby, který implementuje volání služby. |
