...
|– Typ ovládacího prvku|Ovládacích prvků Windows Forms v dokumentech jsou uzavřen do třídy poskytované [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)] udávající další funkce, které jsou specifické pro ovládací prvky pro sešit aplikace Excel nebo dokument aplikace Word. Pokud máte například `Button` řízení na listu aplikace Excel, je nutné zadat typ jako <xref:Microsoft.Office.Tools.Excel.Controls.Button> místo <xref:System.Windows.Forms.Button> při odkazování na nebo přetypování objektu.|  
