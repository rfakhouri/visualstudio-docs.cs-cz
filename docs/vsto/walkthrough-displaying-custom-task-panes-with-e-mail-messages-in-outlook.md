...
# <a name="walkthrough-display-custom-task-panes-with-email-messages-in-outlook"></a>Návod: Zobrazení vlastních podoken úloh s e-mailové zprávy v aplikaci Outlook
  Tento návod ukazuje, jak zobrazit jedinečný instanci vlastního podokna úloh s každou e-mailové zprávy, která je vytvořit nebo otevřít. Uživatele můžete zobrazit nebo skrýt podokno úloh pomocí tlačítka na pásu karet každou e-mailové zprávy.  
...
-   Zobrazení vlastní uživatelské rozhraní pásu karet s e-mailové zprávy.  
...
-   Microsoft [!INCLUDE[Outlook_15_short](../vsto/includes/outlook-15-short-md.md)] nebo aplikace Microsoft Outlook 2010.  
 ...
 ![odkaz na video](../vsto/media/playvideo.gif "odkaz na video") související Videoukázka, najdete v části [jak I: pomocí podokna úloh v aplikaci Outlook?](http://go.microsoft.com/fwlink/?LinkID=130309).  
...
1.  Vytvoření **doplněk aplikace Outlook** projektu s názvem **OutlookMailItemTaskPane**. Použití **doplněk aplikace Outlook** šablona projektu. Další informace najdete v tématu [postupy: vytvoření Office projekty v sadě Visual Studio](../vsto/how-to-create-office-projects-in-visual-studio.md).  
...
## <a name="display-the-custom-ribbon-user-interface-with-email-messages"></a>Zobrazení vlastní uživatelské rozhraní pásu karet s e-mailové zprávy  
...
### <a name="to-display-the-custom-ribbon-ui-with-email-messages"></a>Zobrazit vlastní uživatelské rozhraní pásu karet s e-mailové zprávy  
...
## <a name="create-a-class-to-manage-inspector-windows-and-custom-task-panes"></a>Vytvořte třídu, ke správě systému windows nástroj inspector a vlastních podoken úloh  
 Existuje několik případů, kdy doplňku VSTO musí identifikovat které vlastního podokna úloh je přidružen konkrétní e-mailové zprávy. Tyto případy patří:  
  
-   Když uživatel nezavře e-mailovou zprávu. V takovém případě doplňku VSTO musíte odebrat odpovídající vlastního podokna úloh k zajištění, že jsou správně vyčistit prostředky využívané třídou doplňku VSTO.  
...
## <a name="initialize-and-clean-up-resources-used-by-the-add-in"></a>Inicializace a vyčištění prostředků používaných v doplňku  
 Přidejte kód, který `ThisAddIn` tříd k chybě při inicializaci doplňku VSTO, když je načten a vyčistěte prostředky využívané třídou doplňku VSTO v případě, že je odpojen. Nastavením obslužné rutiny události pro inicializaci doplňku VSTO <xref:Microsoft.Office.Interop.Outlook.InspectorsEvents_Event.NewInspector> událostí a pomocí předání této obslužné rutiny události všechny existující e-mailové zprávy. Když doplňku VSTO je odpojen, odpojte obslužné rutiny události a vyčistit objekty používané doplňku VSTO.  
...
5.  Přidejte následující <xref:Microsoft.Office.Interop.Outlook.InspectorsEvents_Event.NewInspector> obslužná rutina události `ThisAddIn` třídy. Pokud nový <xref:Microsoft.Office.Interop.Outlook.Inspector> obsahuje e-mailovou zprávu, metoda vytvoří instanci novou `InspectorWrapper` objekt, který chcete spravovat vztah mezi e-mailovou zprávu a odpovídající podokna úloh.  
...
## <a name="test-the-project"></a>Testování projektu  
 Při spuštění ladění projekt se otevře aplikace Outlook a že je načtený doplňku VSTO. Doplňku VSTO zobrazí jedinečný instanci vlastního podokna úloh s každou e-mailové zprávy, která je otevřená. Vytvořte několik nových e-mailové zprávy k testování kódu.  
  
### <a name="to-test-the-vsto-add-in"></a>K testování doplňku VSTO  
  
1.  Stiskněte klávesu **F5**.  
  
2.  V aplikaci Outlook, klikněte na tlačítko **nový** vytvořit novou e-mailové zprávy.  
  
3.  Na pásu karet e-mailové zprávy, klikněte na tlačítko **doplňky** a pak klikněte **zobrazit podokno úloha** tlačítko.  
  
     Ověřte, že podokna úloh s názvem **Moje podokna úloh** se zobrazí s e-mailové zprávy.  
...
7.  V aplikaci Outlook, klikněte na tlačítko **nový** vytvořit druhý e-mailové zprávy.  
...
10. Změna výběru první e-mailové zprávě.  
