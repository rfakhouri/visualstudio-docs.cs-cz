...
|**Přepínací tlačítko**|<xref:Microsoft.Office.Tools.Ribbon.RibbonToggleButton>|  
...
### <a name="common-tasks-using-the-properties-of-ribbon-controls"></a>Běžné úlohy pomocí vlastností ovládacích prvků pásu karet  
 ...
 V některých případech stát vlastnosti jen pro čtení po načtení pásu karet, nebo po přidání ovládacího prvku do dynamické nabídky. Další informace najdete v tématu [vlastnosti nastavení se stát jen pro čtení](#SettingReadOnlyProperties).  
...
##  <a name="SettingReadOnlyProperties"></a>Nastavení vlastností, které jen pro čtení  
...
-   V metodě CreateRibbonExtensibilityObject `ThisAddin`, `ThisWorkbook`, nebo `ThisDocument` třídu do projektu.  
  
 Dynamické nabídky poskytují některé výjimky. Můžete vytvořit nové ovládací prvky, nastavit jejich vlastnosti a poté je přidejte do dynamické nabídky v době běhu, i když je načten na pásu karet, která obsahuje v nabídce.  
  
 Kdykoli můžete nastavit vlastnosti pro ovládací prvky, které přidáte do dynamické nabídky.  
  
 Další informace najdete v tématu [vlastnosti se stát jen pro čtení](#ReadOnlyProperties).  
  
### <a name="setting-properties-in-the-constructor-of-the-ribbon"></a>Nastavení vlastností v konstruktoru pásu karet  
...
### <a name="setting-properties-in-the-createribbonextensibilityobject-method"></a>Nastavení vlastností v metodě CreateRibbonExtensibilityObject  
 Můžete nastavit vlastnosti `Ribbon` řízení, pokud přepíšete metodu CreateRibbonExtensibilityObject v `ThisAddin`, `ThisWorkbook`, nebo `ThisDocument` třídu do projektu. Další informace o metodě CreateRibbonExtensibilityObject najdete v tématu [přehled pásu karet](../vsto/ribbon-overview.md).  
  
 Následující příklad nastaví vlastnosti pásu karet v metodě CreateRibbonExtensibilityObject `ThisWorkbook` třída projektu sešitu aplikace Excel.  
