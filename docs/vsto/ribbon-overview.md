---
title: "Přehled pásu karet | Microsoft Docs"
ms.custom: 
ms.date: 02/02/2017
ms.reviewer: 
ms.suite: 
...
## <a name="customizing-the-microsoft-office-ribbon"></a>Přizpůsobení pásu karet aplikace Microsoft Office  
...
## <a name="exporting-a-ribbon-from-the-ribbon-designer-to-ribbon-xml"></a>Export pásu karet z Návrháře pásu karet do kódu XML pásu karet  
...
### <a name="updating-the-code"></a>Aktualizace kódu  
 ...
 Musíte taky přidat kód pro `ThisAddIn`, `ThisWorkbook`, nebo `ThisDocument` třídu, která potlačí metodu CreateRibbonExtensibilityObject a vrátí třídy XML pásu karet do aplikací Office.  
  
 Další informace najdete v tématu [XML karet](../vsto/ribbon-xml.md).  
  
## <a name="adding-multiple-ribbon-items-to-a-project"></a>Přidávání více položek pásu karet do projektu  
...
-   Vyberte, které pásu karet zobrazíte v době běhu.  
  
### <a name="selecting-which-ribbons-to-display-at-run-time"></a>Výběr, který stuhy k zobrazení v době běhu  
 Protože projektu mohou obsahovat více než jeden pásu karet, můžete vybrat, které pásu karet zobrazíte v době běhu.  
  
 Pokud chcete vybrat pásu karet zobrazíte v době běhu, přepíše metodu CreateRibbonExtensibilityObject v `ThisAddin`, `ThisWorkbook`, nebo `ThisDocument` třída projektu a vraťte se na pásu karet, které chcete zobrazit. Následující příklad zkontroluje hodnotu pole s názvem `myCondition` a vrátí odpovídající pásu karet.  
