...
-   Soubor kódu ThisAddIn. Tento soubor obsahuje vygenerovaného `ThisAddIn` třídu pro váš doplňku VSTO. Další informace o této třídy najdete v tématu [Program VSTO doplňků](../vsto/programming-vsto-add-ins.md).  
  
-   Skryté soubory projektu, které nelze přímo upravovat. Další informace najdete v tématu [skryté soubory projektu](#hiddenfiles).  
  
### <a name="temporary-certificates"></a>Dočasné certifikáty  
 Projekty Office také zahrnovat dočasné certifikát s názvem *název projektu*_TemporaryKey.pfx. Tento certifikát umožňuje podepsat během vývoje projektu manifest aplikace a nasazení. Další informace najdete v tématu [udělit vztah důvěryhodnosti řešení pro systém Office](../vsto/granting-trust-to-office-solutions.md) a [řešení zabezpečení Office](../vsto/securing-office-solutions.md).  
...
## <a name="document-designer-in-document-level-projects"></a>Návrhář dokumentu v projekty na úrovni dokumentu  
...
### <a name="excel-worksheets-and-workbooks-in-the-designer"></a>Listy aplikace Excel a sešitů v Návrháři  
 Když list otevřete v návrháři, můžete jej upravovat stejně, jako byste jej otevřeli přímo v aplikaci Excel. Když dvakrát kliknete na buňku listu, přejde buňka do režimu úprav. Pokud dvakrát kliknete na buňku obsahující hostitelského ovládacího prvku, otevře se Editor kódu a sady Visual Studio vytvoří výchozí obslužnou rutinu události pro ovládací prvek. Pokud chcete přejít na jiné listy, můžete kliknout na ouška listů v dolní části návrháře.  
 ...
 K sešitu a každému listu v sešitu je přidružen soubor kódu. Každý kód soubor obsahuje generated *hostitelská položka* třídu, která představuje sešitu nebo listu. Další informace najdete v tématu [automatizace aplikace Excel s použitím rozšířených objektů](../vsto/automating-excel-by-using-extended-objects.md).  
...
### <a name="design-mode-vs-runtime-mode"></a>Režim návrhu oproti režimu runtime  
 ...
 Chcete-li zobrazit dokumentu v *režimu runtime*, musíte otevřít aplikaci a dokumentu mimo Visual Studio. Můžete také sestavit a spustit projekt, čímž se dokument a aplikace automaticky otevřou mimo sadu Visual Studio.  
...
## <a name="code-editor"></a>Editor kódu  
 ...
 Další informace o editoru kódu najdete v tématu [psaní kódu v editoru kódu a textové](/visualstudio/ide/writing-code-in-the-code-and-text-editor). Další informace o tom, jak napsat kód v projektech Office najdete v tématu [psaní kódu v řešeních pro systém Office](../vsto/writing-code-in-office-solutions.md).  
