...
#### <a name="remove-activex-wrappers-when-the-document-is-opened"></a>Při otevření dokumentu odebrat ActiveX obálky  
 ...
 Všimněte si, pokud vaše doplňku VSTO vytvoří dynamických ovládacích prvků při otevření dokumentu, vaše doplňku VSTO již zavolá `GetVstoObject` metoda jako součást procesu vytváření ovládacích prvků. Není potřeba přidat samostatné volání `GetVstoObject` metoda odebrat obálky ActiveX v tomto scénáři.  
