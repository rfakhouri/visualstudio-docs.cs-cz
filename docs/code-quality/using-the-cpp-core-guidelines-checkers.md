...
Při přidávání nové pravidel pro kontrolu C++ základní pokyny, může zvýšit počet upozornění, které jsou vytvářeny pro existující kód. Sady předdefinovaných pravidel můžete použít k filtrování které typy pravidel se povolit. Témata odkazů pro většinu pravidla jsou ve [Visual Studio C++ základní zkontrolujte Reference](code-analysis-for-cpp-corecheck.md).
