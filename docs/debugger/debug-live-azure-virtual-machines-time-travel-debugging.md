...
Doba trvání cesty ladění (TTD) ve verzi preview v sadě Visual Studio Enterprise umožňuje zaznamenávat do webové aplikace spuštěné v Azure virtuální počítač (VM) přesně rekonstruovat a opakováním postupu provádění. TTD integruje do našich Snapshot Debugger nabízí a umožňuje rewind a přehrát každého řádku kódu, ale tolikrát, kolikrát chcete, což pomáhá izolovat a identifikovat problémy, které se můžou vyskytovat jenom v produkčním prostředí.

Zachycení záznamu TTD nebude zastavit aplikaci, ale bude záznam přidat významné režijní náklady spuštěného procesu zpomalení na základě faktorů, které zahrnují procesu velikost a počet aktivních podprocesů.
...
> * Nastavte snímkovacího bodu a shromažďovat záznam doba trvání cesty
> * Spuštění ladění záznam doba trvání cesty
...
1. Otevřete projekt, který chcete shromáždit záznam doba trvání cesty.
...
1. Zvolte **ladit > připojit Snapshot Debugger...** . Vyberte webová aplikace nasazená na virtuálním počítači Azure a účet úložiště Azure. Vyberte **povolit ladění doba trvání cesty** ve verzi preview možnost a potom klikněte na tlačítko **připojit**.
   ...
   **Moduly** okno zobrazuje, když jste načetli všechny moduly pro virtuální počítač Azure (zvolte **ladit > Windows > moduly** otevřete toto okno).

   ![Zkontrolujte okno modulů](../debugger/media/snapshot-modules.png)

## <a name="set-a-snappoint-and-collect-a-time-travel-recording"></a>Nastavte snímkovacího bodu a shromažďovat záznam doba trvání cesty
...
1. Klikněte pravým tlačítkem na ikonu snímkovací bod (prázdný koule) a zvolte **akce**. V okně Nastavení snímků **akce** zaškrtávací políčko. Klikněte **trasu doba trvání cesty na konec této metody** zaškrtávací políčko.
...
## <a name="start-debugging-a-time-travel-recording"></a>Spuštění ladění záznam doba trvání cesty
...
1. Klikněte na odkaz zobrazit snímek otevřete čas cestovní nahrávání v editoru kódu.
   ...
   Každý jednotlivý řádek kódu pomocí něj průběžně zaznamenávají podle TTD můžete spustit **pokračovat** a **Reverse pokračovat** tlačítka. Kromě toho panelu nástrojů ladění je možné použít k **zobrazit další příkaz**, **Krokovat s vnořením**, **Krokovat s přeskočením**, **Krokovat s Vystoupením**,  **Krokovat zpět do**, **krokovat zpět přes**, **kroku zpět**.
...
Pokud je obtížné znovu vytvořit určitého stavu ve vaší aplikaci, zvažte, zda může pomoci použití podmíněné snímkovacího bodu. Podmíněné snímkovací body umožňují předcházet shromažďování záznam doba trvání cesty až do aplikace přejde do požadovaného stavu, například pokud má proměnná určitou hodnotu, kterou chcete zkontrolovat. [Můžete nastavit podmínky, které využívají výrazy a filtry, nebo počtu položek](../debugger/debug-live-azure-apps-troubleshooting.md).

## <a name="next-steps"></a>Další kroky

V tomto kurzu jste zjistili, jak získat záznam doba trvání cesty pro Azure Virtual Machines. Můžete si přečíst další podrobnosti o Snapshot Debugger.
