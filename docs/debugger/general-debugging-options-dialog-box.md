...
- **Vždy spouštět nedůvěryhodné příkazy ze zdrojového serveru bez zobrazení výzvy**: když je povolena podpora zdrojového serveru, toto nastavení potlačí výchozí chování výzvy při spuštění nedůvěryhodný příkaz.
...
- **Fall zpět na Git Credential Manageru ověřování pro všechny žádosti o zdrojový odkaz**: je povolena podpora zdrojového odkazu při a žádost o zdrojový odkaz se nezdaří ověřování, Visual Studio pak volá Git Credential Manageru.
...
**Povolit ladění jazyka JavaScript pro ASP.NET (Chrome, Edge a IE)**: umožňuje ladicímu prostředku skriptů pro aplikace ASP.NET. Při prvním použití v prohlížeči Chrome budete muset přihlásit do prohlížeče povolení rozšíření Chrome, které jste nainstalovali. Zakažte tuto možnost, chcete-li vrátit ke starší verzi chování.    
...
**Použít experimentální způsob, jak spustit ladění jazyka JavaScript v chromu při spuštění sady Visual Studio jako správce**: říká Visual Studio a zkuste nový způsob, jak spustit Chrome během ladění jazyka JavaScript.
...
**Použijte starší C# a vyhodnocovače výrazů jazyka Visual Basic**: ladicí program použije sada Visual Studio 2013 C#vyhodnocovače výrazů /VB spíše než vyhodnocovače výrazů Visual Studio 2015 roslynu.    
  
**Upozornit při opakovaném použití vlastních vizualizérů ladění s potenciálně nebezpečnými procesy (pouze spravované)**: Visual Studio vás upozorní, když používáte vlastní vizualizér, na kterém běží kód v laděném procesu, protože může být spuštěn nezabezpečený kód.  
...
- **Povolit XAML Edit and Continue**: umožňuje použití operace upravit a pokračovat funkce s kódem XAML. 
...
**Povolení funkce upravit a pokračovat**: Povolí funkci upravit a pokračovat při ladění.  
...
- **Použít změny při pokračování (jenom nativní)**: Visual Studio automaticky zkompiluje a použije všechny zbývající kód změny provedené při pokračování procesu ze stavu pozastavení. Pokud není vybrána, můžete použít změny pomocí **použít změny kódu** položku **ladění** nabídky.  
...
**Při zastavení ladění automaticky zavřete konzolu**: říká Visual Studio a zavřete konzolu na konci relace ladění.
