...
### <a name="dynamic-objects"></a>Dynamické objekty  
 Proměnné můžete použít ve výrazech ladicího programu, které jsou zadány staticky jako dynamický. Pokud objekty, které implementují <xref:System.Dynamic.IDynamicMetaObjectProvider> jsou vyhodnocovány v okně Sledování dynamického zobrazení přidání uzlu. Uzel dynamického zobrazení ukazuje členové objektu, ale neumožňuje úpravy hodnot členů.  
