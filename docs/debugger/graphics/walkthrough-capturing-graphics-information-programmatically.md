...
-   Použijte následující kód k připojení k rozhraní ladění DXGI rozhraní IDXGraphicsAnalysis.  
    ...
    ```  
     ...
     Nezapomeňte se podívat `HRESULT` vrácený [DXGIGetDebugInterface1](https://msdn.microsoft.com/library/windows/desktop/dn457937(v=vs.85).aspx) zajistit získat platný rozhraní před jejich použitím:  
