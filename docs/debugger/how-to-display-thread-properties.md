---
title: "Postupy: zobrazení vlastností vlákna | Microsoft Docs"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology:
- vs-ide-debug
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- thread properties
ms.assetid: f0e64c0e-cdb3-4ad4-9723-09934166f3a2
caps.latest.revision: 
author: mikejo5000
ms.author: mikejo
manager: ghogen
ms.workload:
- multiple
ms.openlocfilehash: f6ad7885aa622954354cd211fc4f986191ea2ca1
ms.sourcegitcommit: 32f1a690fc445f9586d53698fc82c7debd784eeb
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 12/22/2017
---
# <a name="how-to-display-thread-properties"></a>Postupy: Zobrazení vlastností vlákna
Chcete-li získat další informace o vlákno uvedené v [zobrazení vláken](../debugger/threads-view.md), použijte [dialogové okno Vlastnosti vlákno](../debugger/thread-properties-dialog-box.md).  
  
### <a name="to-open-a-thread-properties-dialog-box"></a>Chcete-li otevřít dialogové okno vlastností vláken  
  
1.  Vyberte vlákna ze stromu zobrazí v [zobrazení vláken](../debugger/threads-view.md).  
  
2.  Z **zobrazení** nabídce zvolte **vlastnosti**.  
  
 Dialogová okna vlastnosti nejsou modální, takže můžete kliknutím na jiné položky v okně zobrazení a dialogové okno se zobrazí informace o vybrané položky.