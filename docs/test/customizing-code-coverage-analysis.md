---
title: "Přizpůsobení analýzy pokrytí kódu | Microsoft Docs"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
4.  Nyní když spustíte **analýza pokrytí kódu**tento `.runsettings` soubor bude kontrolovat své chování. Nezapomeňte, že je nutné znovu spustit pokrytí kódu: předchozí výsledky pokrytí a barevné zvýrazňování kódu nejsou skryté automaticky při spuštění testů nebo aktualizaci vašeho kódu.  
  
5.  Vlastní nastavení vypnutí a zapnutí, zrušte výběr nebo vyberte soubor v **Test**, **nastavení testu** nabídky.  
  
 ![Test nabídky nastavení se soubor s vlastním nastavením](../test/media/codecoverage-settingsfile.png "CodeCoverage Soubor_nastavení")  
...
```  
 ...
 Pokud `<Include>` je prázdný, pak zpracování pokrytí kódu zahrnuje všechny sestavení (soubory .dll a .exe), které jsou načtené a pro které **PDB** nalezeny soubory, s výjimkou položky, které odpovídají klauzuli v `<Exclude>` seznamu.  
...
3.  **\\( \\)** odpovídá závorkách "(")  
  
4.  **\\\\**odpovídá oddělovač cestu souboru "\\"  
