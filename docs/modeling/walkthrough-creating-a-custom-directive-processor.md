...
## <a name="creating-a-custom-directive-processor"></a>Vytvoření vlastního procesoru direktiv  
 ...
 Vlastní procesor direktiv přidá proměnnou a vlastnost do vygenerované třídy transformace. Direktiva psaní používá <xref:System.CodeDom> třídy vytvořit kód, který modul přidá do třídy generované transformace. <xref:System.CodeDom> Třídy vytvoření kódu v buď Visual C# nebo [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)], v závislosti na jazyce určeném `language` parametr `template` – direktiva. Jazyk procesoru direktiv a jazyk textové šablony, ke které tento procesor direktiv přistupuje, se nemusejí shodovat.  
...
1.  V sadě Visual Studio vytvořte projekt knihovny tříd jazyka C# nebo Visual Basic s názvem CustomDP.  
    ...
    > [!NOTE]
    >  Pokud chcete nainstalovat procesoru direktiv na více než jeden počítač, je vhodnější použít [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] rozšíření (VSIX) projektu a soubor .pkgdef zahrnout do rozšíření. Další informace najdete v tématu [nasazení procesor – direktiva vlastní](../modeling/deploying-a-custom-directive-processor.md).  
...
4.  Pro [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] pouze, otevřete **projektu** nabídky a klikněte na tlačítko **CustomDP vlastnosti**. Na **aplikace** ve **kořenový obor názvů**, odstraňte výchozí hodnota `CustomDP`.  
...
## <a name="registering-the-directive-processor"></a>Registrace procesoru direktiv  
 Než bude možné volat z textové šablony v direktivě [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)], je nutné přidat klíč registru pro procesoru direktiv.  
  
> [!NOTE]
>  Pokud chcete nainstalovat procesoru direktiv na více než jeden počítač, je lepší k definování [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] rozšíření (VSIX), který obsahuje soubor .pkgdef společně s vaší sestavení. Další informace najdete v tématu [nasazení procesor – direktiva vlastní](../modeling/deploying-a-custom-directive-processor.md).  
...
## <a name="testing-the-directive-processor"></a>Testování procesoru direktiv  
 ...
 V tomto příkladu volá textová šablona direktivu a předá název souboru XML, který obsahuje dokumentaci pro soubor třídy.
  
 Textové šablony použije <xref:System.Xml.XmlDocument> vlastnost, která vytvoří direktivu přejděte XML a tisknout dokumentační komentáře.  
...
1.  V **Průzkumníku řešení**, klikněte pravým tlačítkem na TestDP.tt a pak klikněte na tlačítko **spustit nástroj pro vlastní**.  
     ...
     Pro [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] uživatele, TestDP.txt, se nemusí zobrazit v **Průzkumníku řešení** ve výchozím nastavení. Chcete-li zobrazit všechny soubory, které jsou přiřazeny k projektu, otevřete **projektu** nabídky a klikněte na tlačítko **zobrazit všechny soubory**.  
...
## <a name="adding-html-to-generated-text"></a>Přidání kódu HTML k vygenerovanému textu  
