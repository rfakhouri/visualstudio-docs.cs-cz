...
|-------------------|-----------------|--------------|
|Nepodařilo se načíst základní třídu{0}, ze které transformace třídy dědí.|Nastane, pokud nemůžete najít základní třída zadaná v `inherits` parametr – direktiva šablony. Zpráva poskytuje číslo – direktiva šablony.|Ujistěte se, že pro zadanou třídu existuje a že sestavení, které existuje ve je uveden v direktivu sestavení.|
|Nepodařilo se vyřešit obsahují text pro soubor:{0}|Nastane, když nelze najít šablonu zahrnuty. Zpráva obsahuje název požadovaný soubor.|Ujistěte se, že cesta k souboru je relativní k cestě původní šablony, nebo, že soubor je v umístění, která je zaregistrovaná u hostitele nebo že je úplná cesta k souboru.|
...
|---------------------|-----------------|--------------|
|Načítání souboru zahrnout '{0}' vrátil hodnotu null nebo prázdný řetězec.|V případě zahrnuté textového souboru šablony je prázdný. Zpráva obsahuje název souboru souboru zahrnuty.|Buď odeberte – direktiva include nebo zkontrolujte, zda že soubor má nějaký obsah.|
...
|Hodnotu neplatný ladění '{0}' byla zadaná v – direktiva šablony. Ladění hodnota musí být "true" nebo "Nepravda". Použije se výchozí hodnotu "false".|Nastane při `debug` je nesprávně zadán parametr – direktiva šablony. Zpráva poskytuje číslo – direktiva šablony.|Nastavte parametr ladění na "true" nebo "Nepravda".|
|Neplatná hodnota HostSpecific '{0}' byla zadaná v – direktiva šablony. Hodnota HostSpecific musí být "true" nebo "Nepravda". Použije se výchozí hodnotu "false".|Nastane při parametr specifický pro hostitele v `template` – direktiva je nesprávně zadán. Zpráva poskytuje číslo – direktiva šablony.|Nastavte parametr specifický pro hostitele na "true" nebo "Nepravda".|
|Neplatný jazyk '{0}' byla zadaná v – direktiva 'šablony'. Jazyk musí být buď "C" nebo "VB". Použije se výchozí hodnota "C".|Nastane, když je zadán nepodporovaný jazyk v `template` – direktiva. Pouze "C" nebo "VB" jsou povoleny (rozlišování malých a velkých písmen). Zpráva poskytuje číslo – direktiva šablony.|Nastavte `language` parametr – direktiva šablony "C" nebo "VB".|
...
|Nebyl zadán žádný procesoru pro direktivu s názvem '{0}'. Direktiva budou ignorovány.|V případě zadáte `custom` – direktiva, ale nezadáte `processor` atribut. Zpráva obsahuje název direktiva a číslo řádku.|Zadejte `processor` atribut s názvem `directive` procesoru pro direktivu.|
|Procesor s názvem "{0}'nebyla nalezena direktiva s názvem'{1}'. Direktiva budou ignorovány.|Nastane, když systém nemůže najít `directive` procesoru, které jste zadali v rámci `custom` – direktiva. Zpráva poskytuje direktivy název, procesor název a číslo řádku direktivu.|Nastavte `processor` atribut v direktivě k názvu direktivy procesoru.|
|Požadovaný parametr '{0}'pro – direktiva'{1}' nebyla nalezena. Direktiva budou ignorovány.|Nastane, když systém neposkytuje požadovaný parametr direktivy. Zpráva obsahuje název chybějící parametr, direktivy název a číslo řádku.|Zadejte parametr chybí.|
|Procesor s názvem "{0}'nepodporuje direktiva s názvem'{1}'. Direktiva budou ignorovány.|Nastane, když procesoru direktiv nepodporuje direktivu. Zpráva obsahuje název a řádek počtu problematické – direktiva společně s názvem direktivy procesoru.|Opravte název direktivu.|
|Direktiva include souboru '{0}' způsobí, že nekonečné smyčce.|Zobrazí v případě, že direktivy začlenění na kruhový zadávají (například soubor A obsahuje soubor B, který obsahuje soubor A).|Nezadávejte žádné cyklické direktivy začlenění.|
...
|Došlo k problému vytváření a inicializace procesoru pro direktivu s názvem '{1}'. Typ procesoru je {0}. Direktiva budou ignorovány.|Nastane, když systém nemohl vytvořit nebo inicializovat direktivy procesor. Zpráva obsahuje název a řádek číslo směrnice a typ procesoru.|Ujistěte se, že používáte správné procesoru direktiv, a že procesoru direktiv veřejný výchozí konstruktor. Jinak použijte možnosti ladění a zjistěte, proč se nedaří metodu Initialize() direktivy procesoru. Další informace najdete v tématu [řešení potíží s textové šablony](../modeling/debugging-a-t4-text-template.md).|
