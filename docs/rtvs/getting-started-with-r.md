---
title: "Začínáme s R v pro sadu Visual Studio | Microsoft Docs"
ms.custom: 
ms.date: 06/29/2017
ms.reviewer: 
ms.suite: 
...
Až budete mít R nástrojů pro Visual Studio (RTVS) nainstalován (viz [instalace](installation.md)), můžete rychle získat chuť prostředí, které poskytují tyto nástroje. Následující části vás provede krátkou prohlídku:

- [Vytvoření projektu R](#create-an-r-project)
- [Prozkoumat interaktivní okno a IntelliSense](#explore-the-interactive-window-and-intellisense)
- [Funkce pro úpravu kódu prostředí](#experience-code-editing-features)
- [Ladění kódu](#debugging-your-code)
...
1. Po vytvoření projektu, zobrazí se následující windows:
    ...
    - Na pravé straně je Průzkumníka Visual Studio řešení, kde uvidíte projektu uvnitř obsahujícího *řešení*. (Řešení může obsahovat libovolný počet projekty různých typů; viz [projekty](projects.md) podrobnosti.
...
Interaktivních okna také umožňuje zkontrolovat historii, načtěte a uložit pracovní prostory, připojení k ladicí program a interakci s soubory zdrojového kódu místo pomocí kopírování vkládání. V tématu [práce s interaktivních okna R](interactive-repl.md) podrobnosti.
...
Psaní kódu v `.R` souboru umožňuje zobrazit všech vašich kódech najednou a je jednodušší a provést změny malé rychle zjistit výsledek spuštěním kódu v okně interaktivní. Může mít libovolný počet souborů tak, jak chcete v projektu. Pokud je kód v souboru, můžete také spustit ho krok za krokem v ladicím programu (popsané dál v tomto tématu). Tyto funkce jsou užitečné, když jste vývoj výpočetní algoritmy a psaní kódu k manipulaci s jeden nebo více datových sad, zejména v případě, že chcete prověřit všechna mezilehlých výsledků.
...
Je informace, které můžete provést v editoru. Podrobnosti najdete v tématu [úpravy kódu](code-editing.md), [IntelliSense](code-intellisense.md), a [výstřižky kódu](code-snippets.md).
...
Přechod hlubší naleznete v tématu [ladění](debugging.md) a [proměnné Explorer](variable-explorer.md).

## <a name="next-steps"></a>Další kroky

V tomto návodu jste se naučili základy R projektů, použití interaktivních okna, úprav a ladění v sadě Visual Studio code. Chcete-li pokračovat, prohlížení další funkce, najdete na následující témata a také témata uvedené v tabulce obsahu:
