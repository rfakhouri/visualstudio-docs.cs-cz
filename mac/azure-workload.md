---
title: Azure
description: Přidání úložiště dat Azure, ověřování a nabízených oznámení do mobilních aplikací v sadě Visual Studio pro Mac
ms.assetid: 3E5ADF4A-6655-42AD-BFC1-CE5E437C457B
author: asb3993
ms.author: amburns
ms.date: 05/06/2018
ms.openlocfilehash: 58562fdcb01bf82da49c05a06af1028200c5822e
ms.sourcegitcommit: b400528a83bea06d208d95c77282631ae4a93091
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 05/23/2018
ms.locfileid: "34453839"
---
# <a name="azure"></a>Azure

Visual Studio pro Mac umožňuje připojit se k Azure v mnoha různými způsoby:

- [Azure Functions](azure-functions.md)
- [Publikování do služby Azure](https://blog.xamarin.com/publish-azure-visual-studio-mac/)
- [Mobilní aplikace služby](connected-services.md)